...
The baseline registries only carry the static cost and carbon intensity
of a resource version; this ledger adds the independent *signal* side of
the dynamic market. A signal is a regional observation of the live
energy mix, unit cost and carbon intensity with an observation moment
and an expiry moment:

* ``history`` maps each region to its published records, versions
  consecutive from 1 and observation moments strictly increasing;
* ``idempotency`` binds each publish key to the signal it carried;
...
:func:`publish` appends a new version under an idempotency key and
:func:`get` answers the latest signal for a region that has been
observed and has not expired at an evaluation moment. The signal file
is never rewritten by the read paths.
"""
...
_VERSION = 1
_SIGNAL_FIELDS = ("region", "observed_at", "expires_at", "energy_mix",
                  "unit_cost", "carbon_intensity")
_RECORD_FIELDS = ("version",) + _SIGNAL_FIELDS
_ROOT_FIELDS = ("version", "history", "idempotency", "audit")
_EVENT_FIELDS = ("key", "region", "version")
_LOCK_SUFFIX = ".lock"
_ENERGY_TOTAL = 10_000
...
def _check_values(signal: dict[str, Any]) -> None:
    ...
    if not _is_plain_int(observed_at) or observed_at < 0:
        raise ValueError("observed_at must be a non-boolean non-negative "
                         "integer")
    if not _is_plain_int(expires_at) or expires_at < 0:
        raise ValueError("expires_at must be a non-boolean non-negative "
                         "integer")
    if expires_at < observed_at:
        raise ValueError("expires_at must not be earlier than observed_at")

    for name in ("unit_cost", "carbon_intensity"):
        value = signal[name]
        if not _is_plain_int(value) or value < 0:
            raise ValueError(f"{name} must be a non-boolean non-negative "
                             "integer")

    mix = signal["energy_mix"]
    if not isinstance(mix, dict) or not mix:
        raise ValueError("energy_mix must be a non-empty object")
    total = 0
    seen: set[str] = set()
    for source, share in mix.items():
        if not isinstance(source, str) or not source:
            raise ValueError("energy_mix sources must be non-empty strings")
        if source in seen:
            raise ValueError("energy_mix sources must be distinct")
        seen.add(source)
        if not _is_plain_int(share) or share < 0:
            raise ValueError("energy_mix shares must be non-boolean "
                             "non-negative integers")
        total += share
    if total != _ENERGY_TOTAL:
        raise ValueError("energy_mix shares must sum to 10000")
...
def _normalize_signal(signal: object) -> dict[str, Any]:
    if not isinstance(signal, dict) \
        ...
        raise ValueError("signal must be an object with exactly region, "
                         "observed_at, expires_at, energy_mix, unit_cost "
                         "and carbon_intensity")
    normalized = {
        "region": signal["region"],
        "observed_at": signal["observed_at"],
        "expires_at": signal["expires_at"],
        "energy_mix": signal["energy_mix"],
        "unit_cost": signal["unit_cost"],
        "carbon_intensity": signal["carbon_intensity"],
    }
    _check_values(normalized)
    normalized["energy_mix"] = {
        source: normalized["energy_mix"][source]
        for source in sorted(normalized["energy_mix"])}
    return normalized
...
def _validate_structure(data: object) -> tuple[
    ...
    for region, records_raw in history_raw.items():
        ...
        for index, record in enumerate(records_raw, start=1):
            ...
            mix = record["energy_mix"]
            if list(mix) != sorted(mix):
                raise ValueError("energy_mix sources must be ordered by "
                                 "code point")
            if records and record["observed_at"] <= records[-1]["observed_at"]:
                raise ValueError("observed_at must increase across versions")
            records.append({
                "version": index,
                "region": region,
...
def publish(
    ...
    """Persistently publish one regional live signal version.

    ``path`` and ``idempotency_key`` must be non-empty strings. The
    signal must be an object with exactly the six top-level fields
    region, observed_at, expires_at, energy_mix, unit_cost and
    carbon_intensity: a non-empty region string, non-boolean
    non-negative observation and expiry moments with expiry not earlier
    than observation, non-boolean non-negative unit cost and carbon
    intensity, and a non-empty energy_mix object of distinct non-empty
    sources to non-boolean non-negative integer shares summing to exactly
    10000. Any deviation raises ``ValueError`` before the file is
    touched.

    A first publication saves the canonical record -- the signal fields
    plus the next consecutive ``version`` for that region, starting at 1
    -- together with the idempotency binding and a publish audit event
    ...
    the file; the same key with a different signal, or a new version
    whose observation moment does not strictly increase, raises
        ...
        with _process_lock(store.realpath):
            ...
            if existing_region is not None:
                ...
                if {field: existing[field]
                        for field in _SIGNAL_FIELDS} != normalized:
            ...
            if versions is None:
                ...
                if normalized["observed_at"] <= versions[-1]["observed_at"]:
                    raise ValueError("observed_at must increase across "
                                     "versions")
            ...
            record: dict[str, Any] = {
                ...
                "energy_mix": dict(normalized["energy_mix"]),
...
def get(
    ...
    """Return the valid signal for ``region`` at evaluation moment ``at``.

    ``path`` and ``region`` must be non-empty strings and ``at`` a
    non-boolean non-negative integer, else ``ValueError``. The answer is
    the latest version of the region whose observation moment is not
    later than ``at`` and whose expiry moment is not earlier than
    ``at``. A missing signal file raises ``FileNotFoundError``; an
    invalid file -- including non-canonical bytes -- raises
    ``ValueError``; a region with no published signal raises
    ``KeyError(region)`` and a region that has no version still valid at
    ``at`` raises ``LookupError``; any other locking or I/O failure
    raises ``OSError``. The call never writes, and the returned record
    is a fresh copy with version, region, observed_at, expires_at,
    energy_mix, unit_cost and carbon_intensity.
    """
    if not isinstance(path, str) or not path:
        raise ValueError("path must be a non-empty string")
    if not isinstance(region, str) or not region:
    ...
    for record in records:
        if record["observed_at"] <= at <= record["expires_at"]:
            valid = record
    if valid is None:
        raise LookupError(region)
    result = dict(valid)
    result["energy_mix"] = dict(valid["energy_mix"])
    return result
