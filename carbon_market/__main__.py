...
def parser() -> argparse.ArgumentParser:
    ...
    server.add_argument(
        ...
        help="checkpoint file GET /audit/proof anchors proof exports in; "
             "requires --audit")
    return command
...
def main() -> None:
    ...
    if args.command == "serve":
        ...
        # argparse's status 2. --checkpoint enables GET /audit/proof and
        # is only valid together with --audit.
        if args.audit is None:
            if args.token is not None or args.auth is not None \
                    or args.checkpoint is not None:
                command.error("--token, --auth and --checkpoint require "
                              "--audit")
        else:
            if not args.audit:
