...
The supply file is one compact UTF-8 JSON document — version, history sorted by resource id and version, idempotency map sorted by key, audit events — with non-ASCII written through and exactly one trailing newline; negative-zero and non-finite number literals are illegal. Every read entry point (`publish` on an existing file, `get`, `feasible` and `market.clear`) enforces the exact canonical bytes: out-of-order fields or map keys, extra whitespace, ASCII-escaped non-ASCII, and a missing or duplicated trailing newline are all `ValueError`, so a file that was not produced by `publish` can never be read as one. Publishing commits under the per-real-path cross-process exclusive lock with pre-call byte restoration on commit failure, queries take the shared lock, and invalid arguments, values, encodings, JSON, structure, ordering or references raise `ValueError`, missing input files or publish parent directories `FileNotFoundError`, and other locking or I/O failures `OSError`.

## Market clearing ledger

`market.clear(jobs, supply, ledger, job_id, key, at)` settles one accepted job against the versioned supply under a new, separate clearing ledger (its own version-1 document, never the `market.match` ledger). The three paths, `job_id` and `key` must be non-empty strings and `at` a non-boolean non-negative integer; the three paths must resolve to pairwise distinct real paths — any violation raises `ValueError` before a business file is read.

The acceptance file and the supply file are read together as one snapshot — shared locks on both inputs and an exclusive lock on the ledger, all acquired in one global real-path order together with `resources.feasible`, so concurrent callers never deadlock. Only the highest version of each resource valid at `at` (start ≤ at ≤ end) is considered; region, residency, deadline, cost and carbon-budget feasibility are exactly the `resources.feasible` semantics. Each candidate version's capacity is additionally reduced by the work already cleared against that exact `(resource_id, version)` in the ledger — no version is ever oversold, and bookings never borrow capacity across versions. Remaining candidates are ordered by carbon intensity, unit cost and resource id; the first is the unique selected resource.

`clear` returns `(record, created)`; the record carries `job_id`, `at`, the job's `work`, the ordered candidate snapshot (each entry the feasibility entry — resource record and both totals — as seen at clearing time) and a `selected` of `resource_id`/`version` naming the first candidate. The ledger is created only on the first successful clearing, and that one write commits the result record, the idempotency binding and the request-key/resource-version audit event together. The ledger is canonical compact UTF-8 JSON — `version`, `cleared` sorted by job id, `idempotency` and `audit` sorted by key — with non-ASCII written through and exactly one trailing newline, committed with a synced same-directory temporary file, atomic replace and directory fsync; commit failure restores the pre-call bytes (or removes the never-before-existing ledger) and leaves no fragments.

Replaying the same key for the same job and evaluation moment returns the stored record with `False` without reselecting, appending an event or rewriting a byte — even if the supply changed since. The same key with a different job or evaluation moment, or a job already cleared under another key, raises `ValueError` and leaves the ledger byte-for-byte untouched. An unknown job raises `KeyError`; when no resource remains feasible after the version-scoped deductions the call raises `LookupError`; neither ever creates the ledger. A missing input file or ledger parent directory raises `FileNotFoundError`; invalid structure, references, ordering or canonical bytes in any of the three files raise `ValueError`; other locking, read/write or sync failures remain `OSError`. The existing `market.match`, `resources` interfaces, server and HTTP behavior are unchanged.
