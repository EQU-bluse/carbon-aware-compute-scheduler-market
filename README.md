...
A first submission commits a `pending` record first and only then stores both files' raw bytes content-addressed under the ledger directory, all before verification runs, so an evidence-write failure or a restart resumes the same bundle from the persisted `pending` record instead of posing as never received, and leftover fragments are ignored. Success flips the record to `active` and returns `(record, True)`; a format failure quarantines the record and re-raises `BundleFormatError`, a tag, chain or lifecycle mismatch quarantines it and re-raises `BundleMismatchError` — a claimed tag that does not match the checkpoint's bytes included, and in both cases the evidence stays retained. Replays are idempotent: an `active` record replayed with the same bundle and the correct claimed tag returns its copy with `False` and writes nothing; the claimed tag is always rechecked, even for an `active` record, so a same-bundle replay carrying a wrong tag atomically flips the record to `quarantined` (`error` names `BundleMismatchError`), leaves the content-addressed evidence in place and raises `BundleMismatchError`; a `pending` one resumes, a `quarantined` one re-raises the recorded exception type untouched, and the same key with a different bundle retains both bundles' bytes and summaries under `conflicts` (the summary carrying the actual tag as well) without overwriting the main record, raising `ValueError`. Submissions serialize across processes on the ledger directory's exclusive kernel lock and commit through temporary-file writes, fsync, atomic replace and directory fsyncs; the ledger is one compact UTF-8 `ledger.json` (`version`, `records`, `conflicts`, in that order) ending with a single newline.

`acceptance.get(ledger_dir, key)` returns a copy of the record (`KeyError` for an unknown key); `acceptance.search(ledger_dir, cursor=None, limit=100, state=None)` pages the records in key code-point order with a composable state filter and exclusive cursor, returning `{"entries": [[key, record], ...], "next": ...}`. `acceptance.read_key(ledger_dir, key, condition=None)` and `acceptance.read_page(...)` are the conditional variants for the HTTP endpoint: under the ledger's shared lock they perform the same lookup, serialize the response and compute the strong entity tag, and return `(body, etag, not_modified)`; an optional `condition` must be one strong tag and is compared under the same lock, so the 200/304 decision always comes from one complete ledger version. Invalid arguments or an invalid ledger raise `ValueError`, missing inputs or parents `FileNotFoundError`, and other locking or I/O failures `OSError`.
...
The same authorization as `GET /audit` applies, in the same order: a missing, blank or duplicated `X-Audit-Token` gets 401 `unauthorized`, an unknown or expired token 403, and an unreadable or invalid configuration 503 `auth_unavailable` — none of these opens the ledger. `GET /acceptance?key=K` looks up one record; the key stands alone, so combining it with `cursor`, `limit` or `state` is 400 `invalid_request`. Without `key` the query paginates with the exclusive `cursor`, the `limit` page size (1..1000, default 100) and the `state` filter, which only accepts `pending`, `active` or `quarantined`; unknown, repeated or empty parameters and illegal values are 400 `invalid_request` as well. An optional `If-None-Match` may be omitted or appear exactly once as a single strong tag — one double-quoted string of 64 lowercase hexadecimal digits — and is validated together with the query after authorization: an empty, repeated, weak (`W/"…"`), wildcard (`*`) or list value is 400 `invalid_request` without ever reading the ledger. Scope checks follow that validation and precede any ledger read: an exact lookup requires unrestricted operation and stage scopes and a key scope that is unrestricted or names the requested key, while a paginated query requires all three scopes unrestricted — anything else is a plain 403. The fixed request order is thus authentication, query and conditional-header validation, scope judgement, and only then the ledger read.

A successful exact lookup answers 200 with the record object, a successful page with the `{"entries": [[key, record], ...], "next": ...}` object — both in the library's field order, key code-point order, compact UTF-8 and without a trailing newline. Every 200 carries a strong `ETag` header whose value is one double-quoted lowercase hexadecimal SHA-256 of the complete response body bytes; the exact lookup and each page are hashed from their own response bytes, so any visible record, ordering or cursor change changes the tag. When `If-None-Match` equals the current response's tag the answer is 304 with an empty body and that same `ETag`, otherwise it is 200 with the full body; a conditional request for a missing ledger or unknown key still answers the same 404 as without the header. The shared kernel flock covers the complete ledger read, the business-object formation, the response serialization, the digest and the conditional comparison and is released only afterwards, so a request racing a submission decides 200 or 304 entirely from one complete old or complete new ledger, never a mixture. A missing ledger gets 404 `acceptance_not_found`, an unknown key 404 `acceptance_key_not_found`, an invalid ledger 409 `acceptance_invalid`, and other read failures 503 `acceptance_unavailable`; real paths and system messages never appear in an error.
