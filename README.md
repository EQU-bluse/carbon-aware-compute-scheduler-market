...
## Acceptance ledger endpoint

```bash
python -m carbon_market serve --host 127.0.0.1 --port 8000 --audit PATH --auth CONFIG --acceptance DIR
```

`--acceptance` enables `GET /acceptance`, a read-only query against the acceptance ledger fixed at `DIR`; it may only be given together with `--audit` and the multi-token `--auth` method (alone, empty, repeated or paired with the single `--token` it is a usage error, exit status 2, and the server does not listen). Without it the path stays a plain 404 like any other unknown path. Clients can never select the ledger directory through the query.

Authorization follows the same order as the other audit endpoints: a missing, blank or duplicated `X-Audit-Token` is 401 `unauthorized`; an unknown or expired token is 403 `forbidden`; an authorization configuration that cannot be read or validated at request time is 503 `auth_unavailable` — in every case the ledger is never opened. The query takes `key` alone for an exact record lookup, or the pagination filters `cursor` (exclusive), `limit` (1–1000, default 100) and `state` (one of `pending`, `active`, `quarantined`) without it; an unknown, repeated or empty parameter, a `key` combined with any pagination filter, or an illegal state, cursor or page size gets 400 `invalid_request`. The exact lookup requires the token's operation and stage scopes to be `*` and its key scope to be `*` or to name the requested key; the paginated query requires all three scopes to be `*` — any shortfall is 403 `forbidden`, decided before the ledger is ever read.

A successful exact lookup answers 200 with the record object; a successful paginated query answers 200 with the `{"entries": [[key, record], ...], "next": ...}` page object — both exactly as `acceptance.get` and `acceptance.search` return them, compact UTF-8 with field order and key code-point order intact and no trailing newline. Every query takes the shared-lock read-only path, so a request racing a submission observes either the complete old ledger or the complete new one. A missing ledger gets 404 `acceptance_not_found`, an unknown key 404 `acceptance_key_not_found`, an invalid ledger 409 `acceptance_invalid`, and other read failures 503 `acceptance_unavailable`; real paths and system messages never appear in an error.
...
The `carbon_market.acceptance` module complements the trust directory with a persistent ledger of every bundle submission this machine was offered. `acceptance.submit(checkpoint_path, proof_path, etag, trust_dir, ledger_dir, key)` first runs `audit_proof.verify_bundle` — the complete existing verification, trust-directory sequence included — and records the outcome under the idempotency `key`. Each record carries `key`, `etag`, `checkpoint_digest`, `proof_digest`, `state` and `error` in that order; the `etag` is always the actual strong tag recomputed from the checkpoint's raw bytes, never the caller-claimed value; `state` is `pending`, `active` or `quarantined`, and `error` names the public exception class only while quarantined.

A first submission commits a `pending` record and only then stores both files' raw bytes content-addressed under the ledger directory, both before verification runs, so a restart — even one between the record commit and the evidence write — resumes the same bundle from the stored `pending` record instead of treating the submission as never received, and ignores leftover fragments. Success flips the record to `active` and returns `(record, True)`; a format failure quarantines the record and re-raises `BundleFormatError`, a tag, chain or lifecycle mismatch — a claimed tag that contradicts the recomputed one included — quarantines it and re-raises `BundleMismatchError` — in both cases the evidence stays retained. Replays are idempotent: an `active` record returns its copy with `False`, a `pending` one resumes, a `quarantined` one re-raises the recorded exception type untouched, and the same key with a different bundle retains both bundles' bytes and summaries under `conflicts` (the summary's tag again recomputed from its checkpoint bytes) without overwriting the main record, raising `ValueError`. Submissions serialize across processes on the ledger directory's exclusive kernel lock and commit through temporary-file writes, fsync, atomic replace and directory fsyncs; the ledger is one compact UTF-8 `ledger.json` (`version`, `records`, `conflicts`, in that order) ending with a single newline.
