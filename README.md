...
The optional `--trust-dir PATH` keeps a persistent sequence of the snapshots this machine has accepted, so a whole-bundle replay of an older — otherwise self-consistent — pair is rejected as a rollback. Without the option the single-bundle verification above runs unchanged and no directory is created. With it, the existing verification completes first; then, under the trust directory's exclusive lock, the verified snapshot is compared with the retained chain head. The first use of a not-yet-existing directory (its parent must exist) first lands a `preparing` lifecycle record binding the candidate's strong tag, checkpoint digest and candidate node digest, then saves the candidate snapshot, node and chain head, and advances the record to `ready` only once the chain head is committed — every write commits through a temporary-file write, fsync, atomic replace and directory fsyncs, so any interruption leaves exactly one interpretable stage on disk. The same complete bundle idempotently completes a `preparing` directory; a bundle with different content is rejected and every piece of evidence is kept. A `ready` record must bind the chain head, the head node and the current snapshot — a missing chain head or any relation that does not hold fails verification. A legacy directory holding a legal chain head but no lifecycle record is verified across the whole chain and then completed to `ready`; an existing directory holding neither — empty or orphaned — is never taken over. Every verification re-walks the chain from the first node to the head: every node digest, predecessor tag and predecessor node relation is recomputed, every node's checkpoint bytes must exist, hit their content address and pass the complete structure and digest-chain validation, and adjacent snapshots must satisfy the same strict append-only relation a new bundle is held to — history continuity is never inferred from node metadata alone. A later bundle carrying the same strong tag is accepted only after this full-chain inspection and is then an idempotent replay: the proof is fully re-verified and the directory is left untouched, so no historical damage can be masked by the bundle in hand. A new tag is accepted only when its checkpoint is a strict append-only successor of the latest trusted one — existing generations, anchors, fields and values must survive untouched, only new anchors or generations may be appended — so an already retained earlier version, a fork that rewrites old anchors, or a bundle that cannot continue the head fails verification. Each sequence node binds the current tag, the predecessor tag, the checkpoint byte digest and the previous node digest, and the chain head points only at the last complete node, so a restarted process re-checks the real predecessor relation from disk; tampered node content, chain digests, head relations or lifecycle records are never repaired from the bundle in hand, unreferenced fragments never take part in any decision, and a content address already holding different bytes is never overwritten or repaired. Directory updates serialize across processes, and the chain head is always committed before the lifecycle record lands as `ready`.

On success stdout carries one compact UTF-8 JSON line — `{"valid":true,"etag":...,"generation":...,"closed":...,"events":...,"next":...}` with non-ASCII written through and exactly one trailing newline — the exit status is 0 and stderr stays empty. Failures print no partial stdout, only a single compact failure object on stderr: status 2 `invalid_request` (argument or tag format), 3 `invalid_bundle` (encoding, JSON or structure, including proofs without the snapshot binding and malformed trust metadata), 4 `verification_failed` (tag, digest chain, generation, anchor, state, page, snapshot-binding, trust-sequence or trust-lifecycle mismatch) and 5 `bundle_unavailable` (missing files, a missing trust-directory parent, or other locking, open or read errors). Failure objects never leak paths, system messages or input content.
