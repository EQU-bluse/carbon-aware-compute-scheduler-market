...
A successful exact lookup answers 200 with the record object, a successful page with the `{"entries": [[key, record], ...], "next": ...}` object — both in the library's field order, key code-point order, compact UTF-8 and without a trailing newline. The queries only take the ledger's shared-lock read-only path, so a request racing a submission observes either the complete old ledger or the complete new one. A missing ledger gets 404 `acceptance_not_found`, an unknown key 404 `acceptance_key_not_found`, an invalid ledger 409 `acceptance_invalid`, and other read failures 503 `acceptance_unavailable`; real paths and system messages never appear in an error.
